...
Debug defaults to the Android emulator loopback API origin `http://10.0.2.2:8080/`, configurable with `nexaDebugApiBaseUrl`. Release requires `nexaReleaseApiBaseUrl` with a non-local HTTPS root origin. The release task rejects missing, cleartext, local, and obvious placeholder origins. No production Nexa endpoint is committed. The HTTP implementation uses 10-second connect, 20-second read/write, and 30-second call timeouts as foundation configuration.
